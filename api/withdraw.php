...
try {
    ...
    if ($saldo < $monto) {
        $pdo->rollBack();
        registrarAuditoria($pdo, $usuario_id, 'RETIRO_RECHAZADO', "Saldo insuficiente. Intentó retirar: $monto, Saldo: $saldo");
        http_response_code(400);
    ...
    $stmt = $pdo->prepare("INSERT INTO movimientos (usuario_id, tipo, monto) VALUES (?, 'retiro', ?)");
    ...
    registrarAuditoria($pdo, $usuario_id, 'RETIRO_EXITOSO', "Monto: $monto. Nuevo saldo: $nuevoSaldo");
